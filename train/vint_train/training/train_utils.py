...
def _compute_losses_nomad_with_pos(
    ema_model,
    noise_scheduler,
    batch_obs_images,
    batch_goal_pos,
...
def train_nomad_with_position(
    ...
    with tqdm.tqdm(dataloader, desc="Train Batch", leave=False) as tepoch:
        for i, data in enumerate(tepoch):
            ...
            obsgoal_cond = model("vision_encoder", obs_img=batch_obs_images, goal_pos=batch_goal_pos, input_goal_mask=goal_mask)
            ...
            if i % print_log_freq == 0:
                losses = _compute_losses_nomad_with_pos(
...
def evaluate_nomad_with_position(
    eval_type: str,
    ema_model: EMAModel,
    dataloader: DataLoader,
    transform: transforms,
    ...
    with tqdm.tqdm(
        ...
        for i, data in enumerate(tepoch):
            ...
            rand_mask_cond = ema_model("vision_encoder", obs_img=batch_obs_images, goal_pos=batch_goal_pos, input_goal_mask=rand_goal_mask)

            obsgoal_cond = ema_model("vision_encoder", obs_img=batch_obs_images, goal_pos=batch_goal_pos, input_goal_mask=no_mask)
            obsgoal_cond = obsgoal_cond.flatten(start_dim=1)

            goal_mask_cond = ema_model("vision_encoder", obs_img=batch_obs_images, goal_pos=batch_goal_pos, input_goal_mask=goal_mask)
...
def model_output_with_pos(
    ...
    goal_mask = torch.ones((batch_goal_pos.shape[0],)).long().to(device)
    obs_cond = model("vision_encoder", obs_img=batch_obs_images, goal_pos=batch_goal_pos, input_goal_mask=goal_mask)
    # obs_cond = obs_cond.flatten(start_dim=1)
    obs_cond = obs_cond.repeat_interleave(num_samples, dim=0)

    no_mask = torch.zeros((batch_goal_pos.shape[0],)).long().to(device)
    obsgoal_cond = model("vision_encoder", obs_img=batch_obs_images, goal_pos=batch_goal_pos, input_goal_mask=no_mask)
    # obsgoal_cond = obsgoal_cond.flatten(start_dim=1)  
...
def visualize_diffusion_action_distribution(
    ...
    batch_goal_pos_list = torch.split(batch_goal_pos, max_batch_size, dim=0)
